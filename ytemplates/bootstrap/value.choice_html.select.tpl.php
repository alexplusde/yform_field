...
<?php $choiceOutput = function (rex_yform_choice_view $view) {
    ?>
<option value="<?= rex_escape($view->getValue()) ?>" <?= in_array($view->getValue(), $this->getValue(), true) ? ' selected="selected"' : '' ?>
...
<div<?= rex_string::buildAttributes($groupAttributes) ?>>
    ...
    <div class="rex-select-style">
        ...
        <select<?= rex_string::buildAttributes($elementAttributes) ?>>
            ...
            <option value=""><?= rex_escape($choiceList->getPlaceholder()) ?>
            </option>
            <?php endif ?>
    ...
    <?php if ($notices): ?>
    <p class="help-block small"><?= implode('<br />', $notices) ?>
    </p>
    <?php endif ?>
    </div>